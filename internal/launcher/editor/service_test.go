...
func TestIssueEditorAppliesRepositoryUpdateFromEditedDocument(t *testing.T) {
	...
	updated, err := repo.Issue(context.Background(), "tm-7")
	if err != nil {
...
func TestIssueEditorNoChangesSkipsRepositoryUpdate(t *testing.T) {
	...
	// Phase 1: prepare.
	prepared, err := service.PrepareDocument(context.Background(), "tm-8")
	if err != nil {
		t.Fatalf("PrepareDocument returned error: %v", err)
	}
	defer func() { _ = os.Remove(prepared.TempPath) }()
	...
	unchanged, err := repo.Issue(context.Background(), "tm-8")
	if err != nil {
...
func TestIssueEditorApplyEditsReturnsParseError(t *testing.T) {
	...
	// Phase 1: prepare.
	prepared, err := service.PrepareDocument(context.Background(), "tm-9")
	if err != nil {
		t.Fatalf("PrepareDocument returned error: %v", err)
	}
	defer func() { _ = os.Remove(prepared.TempPath) }()
...
func TestIssueEditorTempFileRemovedAfterApplyEdits(t *testing.T) {
	...
	prepared, err := service.PrepareDocument(context.Background(), "tm-10")
	if err != nil {
		t.Fatalf("PrepareDocument returned error: %v", err)
	}
