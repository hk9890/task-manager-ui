...
func TestServiceLaunchInterpolatesIssueContextAndDelegatesRunner(t *testing.T) {
	...
	if call.Env[0] != "TASKMGR_UI_ISSUE_ID=tm-77" || call.Env[4] != "TASKMGR_UI_PROJECT_ROOT=/repo/root" {
...
func TestServiceBuiltInDefinitionsForV1Actions(t *testing.T) {
	...
	issue := domain.IssueDetail{Summary: domain.IssueSummary{ID: "tm-42", Title: "Launcher task"}}
