...
// test_gateway_test.go provides a thin test helper for the app package's test
...
// appTestGateway bundles a memory repository (for seeding) and an
...
// newTestGateway creates an appTestGateway with an empty memory repository.
func newTestGateway() *appTestGateway {
	repo := memoryrepo.New()
	return &appTestGateway{
		repo:                     repo,
...
func (g *appTestGateway) hasCall(m repository.Method) bool {
...
func (g *appTestGateway) callCountSince(start int, m repository.Method) int {
...
func (g *appTestGateway) hasDashboardCall() bool { return g.hasCall(repository.MethodDashboard) }

// hasIssueCall reports whether an Issue (detail fetch) call appears.
func (g *appTestGateway) hasIssueCall() bool { return g.hasCall(repository.MethodIssue) }

// hasSearchCall reports whether a Search call appears.
func (g *appTestGateway) hasSearchCall() bool { return g.hasCall(repository.MethodSearch) }

// hasUpdateIssueCall reports whether an UpdateIssue call appears.
func (g *appTestGateway) hasUpdateIssueCall() bool { return g.hasCall(repository.MethodUpdateIssue) }

// hasCatalogsCall reports whether a Catalogs call appears.
func (g *appTestGateway) hasCatalogsCall() bool { return g.hasCall(repository.MethodCatalogs) }

// hasCreateIssueCall reports whether a CreateIssue call appears.
func (g *appTestGateway) hasCreateIssueCall() bool { return g.hasCall(repository.MethodCreateIssue) }

// hasCloseIssueCall reports whether a CloseIssue call appears.
func (g *appTestGateway) hasCloseIssueCall() bool { return g.hasCall(repository.MethodCloseIssue) }

// hasAddCommentCall reports whether an AddComment call appears.
func (g *appTestGateway) hasAddCommentCall() bool { return g.hasCall(repository.MethodAddComment) }

// hasHealthCheckCall reports whether a HealthCheck call appears.
func (g *appTestGateway) hasHealthCheckCall() bool { return g.hasCall(repository.MethodHealthCheck) }
...
func (g *appTestGateway) seedSearchResult(iss memoryrepo.Issue) {
	g.repo.Seed(iss)
}

// callCount returns the total number of recorded calls.
func (g *appTestGateway) callCount() int { return len(g.ErrorInjectingRepository.Calls()) }

// resetMark returns the current call count as a "reset mark" for measuring
// subsequent calls via callCountSince/hasCallSince.
func (g *appTestGateway) resetMark() int { return len(g.ErrorInjectingRepository.Calls()) }

// hasCallSince reports whether method m was called after the given mark.
func (g *appTestGateway) hasCallSince(mark int, m repository.Method) bool {
...
func (g *appTestGateway) seedIssueSummary(s domain.IssueSummary) {
...
func (g *appTestGateway) seedIssueDetail(d domain.IssueDetail) {
...
func (g *appTestGateway) seedCatalogs(statuses []domain.StatusOption, types []domain.TypeOption, labels []domain.LabelOption) {
...
func (g *appTestGateway) issueState(id string) *domain.IssueDetail {
...
func (g *appTestGateway) seedDepBlocked(id, title string, issueType string, priority int, extra ...func(*memoryrepo.Issue)) {
...
func (g *appTestGateway) seedReady(id, title string, issueType string, priority int, extra ...func(*memoryrepo.Issue)) {
...
func (g *appTestGateway) seedInProgress(id, title string, issueType string, priority int, extra ...func(*memoryrepo.Issue)) {
