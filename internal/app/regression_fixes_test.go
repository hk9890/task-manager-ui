...
func TestMutationUpdateClearingAssigneeUnassigns(t *testing.T) {
	...
	state := mutationDialogState{
		kind:  mutationUpdate,
		issue: domain.IssueSummary{ID: "bw-1", Title: "Task", Status: "open", Type: "task", Assignee: "alice"},
	}

	newServices := func(t *testing.T) (*recordingUpdateRepository, Services) {
		t.Helper()
		gw := newTestRepository()
		gw.seedIssueSummary(domain.IssueSummary{ID: "bw-1", Title: "Task", Status: "open", Type: "task", Assignee: "alice"})
...
func TestMutationUpdatePriorityRangeEnforced(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			gw := newTestRepository()
			gw.seedIssueSummary(domain.IssueSummary{ID: "bw-1", Title: "Task", Status: "open", Type: "task"})
			services, err := NewServices(gw, config.Default(), t.TempDir())
			if err != nil {
				t.Fatalf("NewServices: %v", err)
			}
			...
			state := mutationDialogState{
				kind:  mutationUpdate,
				issue: domain.IssueSummary{ID: "bw-1", Title: "Task", Status: "open", Type: "task"},
			}
...
func TestModeCyclePrevFromBoardKeepsSelectionAndAllowsEscape(t *testing.T) {
	...
	if got := firstSelectionID(m, mode.Board); got != "bw-1" {
		t.Fatalf("expected board selection bw-1 after init, got %q", got)
	}
	...
	if sel == nil || sel.Issue.ID != "bw-1" {
		t.Fatalf("expected selection preserved as bw-1 after cycling into Detail, got %#v (regression: lastBrowse clobbered to Detail)", sel)
	}

	// Drain the detail load so Detail reflects the preserved selection.
	m = applyMessages(t, m, runBatch(cmd))
	if m.detail.TargetID != "bw-1" && m.detail.Detail.Summary.ID != "bw-1" {
		t.Fatalf("expected Detail to track bw-1 selection, target=%q detail=%q", m.detail.TargetID, m.detail.Detail.Summary.ID)
	}
...
func TestEditPreparedBuildEditorCmdErrorRemovesTempFile(t *testing.T) {
	...
	next, _ := m.Update(editIssuePreparedMsg{
		issueID:  "bw-1",
		prepared: launchereditor.Prepared{IssueID: "bw-1", TempPath: tmpPath},
	})
	m = next.(Model)
