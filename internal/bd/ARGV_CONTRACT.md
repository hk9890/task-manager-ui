...
Each row maps `<consumer>` → `<repository method>` → `<exact argv shape>`.
Produced by audit task `beads-workbench-ppja.2`.

## Methodology

Static analysis: traced every `runner.Run`/`RunJSON`/`RunJSONInto` callsite in
`internal/repository/beads/read_repository.go` and `internal/repository/beads/lean_writes.go`
back through repository methods to UI consumers in `internal/app/model.go`,
...
codebase; the `TestBoardInitRealRepositorySubprocessArgvCardinality` test provides
the only current dynamic cross-check (see Pinning Test Coverage below).

## Read operations

| Consumer | Repository method | argv shape | Source location |
|---|---|---|---|
| `app.Model.Init` (startup) | `HealthCheck` | `bd ping --json` | `internal/repository/beads/read_repository.go:72` |
...
| `app.loadMutationCatalogsCmd` (create/update/comment) | `TypeCatalog` | `bd types --json` | `internal/repository/beads/read_repository.go:687` |
| `app.loadMutationCatalogsCmd` (create/update/comment) | `LabelCatalog` | `bd label list-all --json` | `internal/repository/beads/read_repository.go:708` |
| `app.loadStatusCatalogForIssueCmd` (status/priority quick action) | `StatusCatalog` | `bd statuses --json` | `internal/repository/beads/read_repository.go:666` |

## Write operations

| Consumer | Repository method | argv shape | Source location |
|---|---|---|---|
| `app.submitMutationCmd` (mutationCreate) | `CreateIssue` | `bd create --json --title <title> [--description <desc>] [--type <type>] [--priority <N>] [--assignee <name>] [--labels <csv>]` | `internal/repository/beads/lean_writes.go:32–58` |
| `app.submitMutationCmd` (mutationUpdate) | `UpdateIssue` | `bd update <issueID> [--title <t>] [--description <d>] [--status <s>] [--type <t>] [--priority <N>] [--assignee <name>] [--set-labels <csv>]` | `internal/repository/beads/lean_writes.go:79–115` |
| `app.submitMutationCmd` (mutationStatus quick action) | `UpdateIssue` | `bd update <issueID> --status <status>` | `internal/repository/beads/lean_writes.go:79–115`, triggered at `internal/app/model.go:1701` |
| `app.submitMutationCmd` (mutationPriority quick action) | `UpdateIssue` | `bd update <issueID> --priority <N>` | `internal/repository/beads/lean_writes.go:79–115`, triggered at `internal/app/model.go:1720` |
| `app.submitMutationCmd` (mutationClose) | `CloseIssue` | `bd close <issueID> [--reason <reason>]` | `internal/repository/beads/lean_writes.go:129–140` |
| `app.submitMutationCmd` (mutationComment) | `AddComment` | `bd comments add <issueID> <body>` | `internal/repository/beads/lean_writes.go:153–158` |
| `launcher/editor.IssueEditor.EditIssue` | `UpdateIssue` | `bd update <issueID> [--title <t>] [--description <d>] [--status <s>] [--type <t>] [--priority <N>] [--assignee <name>] [--set-labels <csv>]` | `internal/repository/beads/lean_writes.go:79–115`, triggered at `internal/launcher/editor/service.go:91` |
...
Verified against real bd 1.0.4 in a fresh workspace. See `CountIssues` in `read_repository.go` for the workaround.
...
**Consequence for `CountIssues`**: when `len(query.Statuses) > 1`, the repository omits `--status` entirely and fetches all groups, then filters in-memory to the requested set. Single-status and no-status queries continue to pass `--status` to bd count unchanged.

**Consequence for `SearchIssues`** (text-search path via `bd search`): multi-status queries are not passed to bd. The `bd search` path is only invoked for non-empty text with `WorkState=Any`; in that context the repository passes the first status only. In practice, no production call site passes more than one status to the `bd search` path.

## Distinct argv shape count

Total distinct argv shapes (counting conditional-flag variants as one shape each per repository method path): **19 shapes across 13 repository methods**.
...
| `bd types --json` | YES — `TestTypeCatalogArgvShape` (ppja.3) | `internal/repository/beads/read_repository_test.go` |
| `bd label list-all --json` | YES — `TestLabelCatalogArgvShape` (ppja.3) | `internal/repository/beads/read_repository_test.go` |
| `bd count --by-status --json` (no status filter) | YES — `TestCountIssuesNoStatusFilterArgvShape` (ppja.3) | `internal/repository/beads/read_repository_test.go` |
| `bd create --json --title <t> [optional flags]` | YES — `TestRepositoryCreateIssueMapsCommandArgs` + `TestRepositoryCreateIssueIncludesExplicitZeroPriority` | `internal/repository/beads/writes_test.go` |
| `bd update <id> [field flags]` | YES — `TestRepositoryUpdateIssueMapsCommandArgs` + `TestRepositoryUpdateIssueClearsLabelsWhenRequested` | `internal/repository/beads/writes_test.go` |
| `bd close <id> [--reason <r>]` | YES — `TestRepositoryCloseIssueMapsCommandArgs` | `internal/repository/beads/writes_test.go` |
| `bd comments add <id> <body>` | YES — `TestRepositoryAddCommentMapsCommandArgs` | `internal/repository/beads/writes_test.go` |
...
13. `ListIssues` — method exposed but not called by any UI consumer; no test added (confirmed unreachable in current UI; argv pinned via `TestRepositoryListIssuesBuildsCommandAndMapsSummaries`)
...
16. `HealthCheck` — already pinned via package-internal `testRecordingExecutor` (`TestRepositoryHealthCheckIssuesPingJSON`); import-cycle workaround still in place
