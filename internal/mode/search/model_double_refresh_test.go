...
// newSettledSearchModel creates a search model with the given repository, calls
...
// time while the first reload is still in-flight is a no-op: no new repository call
...
// triggerSearchPreservingSelection() unconditionally, firing a new repository call
