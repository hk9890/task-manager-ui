...
func TestDetailsTeatestFocusSwitchRetainsScrollOffsets(t *testing.T) {
	...
	m.SelectionID = "bw-1"
	m.TargetID = "bw-1"
	m.ApplyLoadedDetail("bw-1", issueWithLongContent("bw-1", "Issue A"))
...
// This overlaps intentionally with the beads-workbench-db0z.7 fix — defense in depth.
func TestDetailsTeatestSelectionChangeResetsAllScrollOffsets(t *testing.T) {
	...
	m.SelectionID = "bw-1"
	m.TargetID = "bw-1"
	issueA := issueWithLongContent("bw-1", "Issue A")
	...
	issueB := issueWithLongContent("bw-2", "Issue B")
	mid.m.ApplyLoadedDetail("bw-2", issueB)
