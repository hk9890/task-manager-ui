...
func newPopulatedRepo() *memoryrepo.Repository {
	...
	repo.Seed(memoryrepo.Issue{ID: "tm-4", Title: "Closed one", Status: "closed", Priority: 1})
	repo.SeedClosed("tm-4", closedAt, "done")
	return repo
}
...
func TestBoardPendingResultsNeverGoesNegativeUnderKeySpam(t *testing.T) {
	...
	for _, wantID := range []string{"tm-1", "tm-2", "tm-3"} {
...
func TestBoardDashboardMsgProcessed(t *testing.T) {
	...
	if len(readyCol.issues) == 0 {
		t.Errorf("expected Ready column to have tm-1, got empty")
	}
	if len(inProgCol.issues) == 0 {
		t.Errorf("expected InProgress column to have tm-2, got empty")
	}
