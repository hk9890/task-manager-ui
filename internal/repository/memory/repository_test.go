...
func TestIssue_RoundTrip(t *testing.T) {
	...
	detail, err := r.Issue(context.Background(), "taskmgr-1")
	if err != nil {
		t.Fatalf("Issue: unexpected error %v", err)
	}
	if detail.Summary.ID != "taskmgr-1" {
		t.Errorf("ID: want taskmgr-1, got %s", detail.Summary.ID)
	}
...
func TestSeedComments(t *testing.T) {
	...
	r := newRepo(base)
	r.Seed(memory.Issue{ID: "taskmgr-1", Title: "issue"})

	r.SeedComments("taskmgr-1",
...
func TestUpdateIssue_MergeFields(t *testing.T) {
	...
	r2.Seed(memory.Issue{ID: "taskmgr-2", Title: "Old title", Status: "open", Labels: []string{"old"}})

	err := r2.UpdateIssue(context.Background(), "taskmgr-2", domain.UpdateIssueInput{
		Title:  ptr("New title"),
	...
	detail, err := r2.Issue(context.Background(), "taskmgr-2")
	if err != nil {
		t.Fatalf("Issue: %v", err)
	}
...
func TestUpdateIssue_NilFieldsPreserved(t *testing.T) {
	...
	detail, _ := r.Issue(context.Background(), "taskmgr-3")
...
func TestCloseIssue_HappyPath(t *testing.T) {
	...
	r.Seed(memory.Issue{ID: "taskmgr-4", Title: "t", Status: "open"})

	err := r.CloseIssue(context.Background(), "taskmgr-4", domain.CloseIssueInput{Reason: "done"})
	if err != nil {
		t.Fatalf("CloseIssue: %v", err)
	}

	detail, err := r.Issue(context.Background(), "taskmgr-4")
	if err != nil {
		t.Fatalf("Issue: %v", err)
	}
...
func TestAddComment_HappyPath(t *testing.T) {
	...
	r := newRepo(base)
	r.Seed(memory.Issue{ID: "taskmgr-6", Title: "t"})

	err := r.AddComment(context.Background(), "taskmgr-6", domain.AddCommentInput{Body: "first comment"})
	if err != nil {
		t.Fatalf("AddComment: %v", err)
	}

	detail, _ := r.Issue(context.Background(), "taskmgr-6")
...
func TestDashboard_ReadyVsBlocked(t *testing.T) {
	...
	// dep closed → taskmgr-open is ready.
	r.Seed(memory.Issue{ID: "dep-1", Status: "closed"})
	r.Seed(memory.Issue{ID: "taskmgr-open", Status: "open", DependsOn: []string{"dep-1"}})

	// dep open → taskmgr-waiting is dep-blocked.
	r.Seed(memory.Issue{ID: "dep-2", Status: "open"})
	r.Seed(memory.Issue{ID: "taskmgr-waiting", Status: "open", DependsOn: []string{"dep-2"}})

	// No deps → taskmgr-nodep is ready.
	r.Seed(memory.Issue{ID: "taskmgr-nodep", Status: "open"})

	// Status == "blocked" (stored, no deps) → in DashboardData.Blocked but NOT in
	// ReadyExplain.Ready. taskmgr ready --explain only includes status=open issues in
	// the ready set (per taskmgr 1.0.4 "taskmgr ready --help" and contract test bwf-5).
	r.Seed(memory.Issue{ID: "taskmgr-status-blocked", Status: "blocked"})
	...
	if !containsID(readyIDs, "taskmgr-open") {
		t.Errorf("ReadyExplain.Ready: expected taskmgr-open, got %v", readyIDs)
	}
	if !containsID(readyIDs, "taskmgr-nodep") {
		t.Errorf("ReadyExplain.Ready: expected taskmgr-nodep, got %v", readyIDs)
	}
	// dep-blocked issue should not be ready.
	if containsID(readyIDs, "taskmgr-waiting") {
		t.Errorf("ReadyExplain.Ready: taskmgr-waiting should not be ready, got %v", readyIDs)
	}
	// taskmgr-status-blocked has stored status="blocked" with no deps. taskmgr ready explicitly
	// excludes non-open-status issues from Ready (per interface.go postcondition).
	if containsID(readyIDs, "taskmgr-status-blocked") {
		t.Errorf("ReadyExplain.Ready: taskmgr-status-blocked (stored-blocked) must NOT be in Ready, got %v", readyIDs)
	}

	// taskmgr-waiting should be in ReadyExplain.Blocked.
	blockedIDs := blockedViewIDs(d.ReadyExplain.Blocked)
	if !containsID(blockedIDs, "taskmgr-waiting") {
		t.Errorf("ReadyExplain.Blocked: expected taskmgr-waiting, got %v", blockedIDs)
	}

	// DashboardData.Blocked should only contain status=="blocked" issues.
	blockedSummaryIDs := issueIDs(d.Blocked)
	if !containsID(blockedSummaryIDs, "taskmgr-status-blocked") {
		t.Errorf("Blocked: expected taskmgr-status-blocked, got %v", blockedSummaryIDs)
	}
	// taskmgr-waiting has status "open", not "blocked".
	if containsID(blockedSummaryIDs, "taskmgr-waiting") {
		t.Errorf("Blocked: taskmgr-waiting (status=open) should not be in Blocked, got %v", blockedSummaryIDs)
	}
}
