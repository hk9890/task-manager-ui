...
// Groups contains only non-zero status entries (zero-count groups are omitted by taskmgr count).
...
type QueryOptions struct {
	...
	Limit int
	// Offset is used with Limit for paginated callers (taskmgr receives Limit+Offset, caller receives page).
	Offset int
	// IncludeClosed includes closed issues in results (maps to taskmgr's -a flag).
	IncludeClosed bool
	// SortBy identifies the sort field. An empty/zero value means no sort override.
	SortBy SortField
	// SortOrder controls ascending vs descending. Descending maps to taskmgr's -r flag.
	SortOrder SortDirection
}
