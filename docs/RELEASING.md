...
- **SLSA build provenance** is generated by `actions/attest-build-provenance`
  and verifiable with `gh attestation verify`. The step runs `continue-on-error`
  as a safety net: the feature is unavailable on user-owned *private*
  repositories (where it fails soft) and starts working once the repository is
  public or org-owned.
...
7. Watch the run complete:
   ...
   notes. The SLSA provenance step fails-soft on a private repo (see
